...
cors = CORS(app, resources={r"/*": {"origins": ["http://127.0.0.1*", "https://accounts.spotify.com/authorize*"]}})
...
def callback():
    ...
    if auth_code is None or phone_number is None:
        return Response('Error: invalid auth code or phone number', status=400, mimetype='application/json')
